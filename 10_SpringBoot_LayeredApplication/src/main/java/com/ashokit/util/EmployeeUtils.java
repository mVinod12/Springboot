...
public class EmployeeUtils {
	...
	public List<Employee> getAllEmployeeInfo(){
		...
		try(
				...
				//reading the data as a line by line
				BufferedReader br = new BufferedReader(fr);
				){
			  //need to define the logic to read the data from file
			String currentRecord;
			while((currentRecord = br.readLine()) !=null) {
				//getting the current line and spliting based on comma separate value
				String[] employeeValues = currentRecord.split(",");
				
				//extract the eemployee values from each record
